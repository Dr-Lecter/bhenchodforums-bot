# name: discord bot
# about: Integrate Discord Bots with BhenchodForums
# version: 0.1
# authors: Dr. Lecter
# url: https://github.com/Dr-Lecter/bhenchodforums-bot
...
after_initialize do
  ...
  def run_bot
    ...
    bot.ready do |event|
      ...
      bot.send_message(SiteSetting.discord_bot_admin_channel_id, "The BhenchodForums role bot has started his shift!")
    end

    # '!kick' - a command to kick members beneath a certain trust level on Discourse

    bot.command(:kick, min_args: 0, max_args: 1, bucket: :admin_tasks, rate_limit_message: 'Hold on cow(girl/boy), rate limit hit!', required_roles: [SiteSetting.discord_bot_admin_role_id], description: 'Block users whose trust level is below a certain integer on discourse') do |event, min_trust_level|

      if !min_trust_level then min_trust_level = 3 end

      discordusers = []

      event.respond "BhenchodForums Role Kick:  Starting.  Minimum Trust Level = #{min_trust_level.to_s}"
      event.respond "BhenchodForums Role Kick:  Starting.  Please be patient, I'm rate limited to respect Discord services."
      event.respond "BhenchodForums Role Kick:  Preparing list of users who also have a registered account on Discord ..."
      ...
      event.respond "BhenchodForums Kick:  Determining user trust levels ..."
      ...
      event.respond "BhenchodForums Kick:  Compiling list of untrusted users ..."
      ...
      if can_do_the_magic_dance == true
        ...
        if ut_count > 0
          untrusted_users.each_with_index do |user, index|
            ...
            event.server.kick(user_id.to_s, "Kicked for not having sufficient trust level on BhenchodForums site")
            event.respond " BhenchodKick:  [#{index + 1}/#{ut_count}] <@#{user_id.to_s}> has been kicked for having insufficient trust level on BhenchodForums Site"
            sleep(SiteSetting.discord_bot_rate_limit_delay)
            rescue => e
              event.respond 'BhenchodForums Kick:  The user you are trying to kick has a role higher than/equal to me.'
          ...
          event.respond 'BhenchodForums Kick:  Great news!  There were no users below the specified or default trust level!'
        end
      else
        event.respond 'BhenchodForums Kick:  Sorry, but I do not have the "Kick Members" permission'
      end

      event.respond "BhenchodForums Kick:  I'm done with the dirty work!"
    end

    # '!sync' - a command to pull all the groups that Discord using members are a member of and set them up on Discord inc. adding those Roles to users accordingly

    bot.command(:sync, min_args: 0, max_args: 3, bucket: :admin_tasks, rate_limit_message: 'Hold on cow(girl/boy), rate limit hit!', required_roles: [SiteSetting.discord_bot_admin_role_id], description: 'Block users whose trust level is below a certain integer on discourse') do |event, clean_house, max_group_visibility, include_automated_groups|
      ...
      event.respond "BhenchodForums Sync:  Starting.  Please be patient, I'm rate limited to respect Discord services."
      event.respond "BhenchodForums Sync:  Checking if there are any eligible groups for sync ..."
      ...
      event.respond "BhenchodForums Sync: #{eligiblegroupbuilder.query.count} eligible group(s) were found"
      ...
      unless eligiblegroupbuilder.query.count == 0
        ...
        event.respond "BhenchodForums Sync:  Preparing list of users who also have a registered account on Discord ..."
        ...
        event.respond "BhenchodForums Sync:  Preparing list of groups that users who have a registered account on Discord belong to on BhenchodForums ..."
        ...
        event.respond "BhenchodForums Sync: #{discourse_groups.length} eligible group(s) were found with Discord users"

        unless discourse_groups.length == 0

          event.respond "BhenchodForums Sync:  Retrieving list of roles from Discord server ..."
          ...
          if clean_house.to_s.downcase == "true"
            ...
            event.respond "BhenchodForums Sync:  Deleting existing mapping roles ..."

            discourse_groups_count = discourse_groups.count

            discourse_groups.each_with_index do |g, index|

              event.respond "BhenchodForums Sync:  [#{index + 1}/#{discourse_groups_count}] Attempting to delete Role"
              ...
              unless role_id.nil?
                begin
                  event.server.role(role_id).delete("BhenchodForums Sync Cleanup")
                  event.respond "BhenchodForums Sync:  Role '#{g[:discourse_name]}' deleted as part of cleanup"
                  sleep(SiteSetting.discord_bot_rate_limit_delay)
                rescue => e
                  event.respond 'BhenchodForums Sync:  I dont appear to have rights to do this though!'
          ...
          event.respond "BhenchodForums Sync:  Creating missing Roles on Discord server ..."
          ...
          discourse_groups.each_with_index do |g, index|
            ...
            event.respond "BhenchodForums Sync:  [#{index + 1}/#{discourse_groups_count}] Attempting to create Role for #{g[:discourse_name]}"

            if !discord_roles.any?{|hash| hash[:name] == g[:discourse_name]}
            begin
              event.server.create_role(name: g[:discourse_name])
              event.respond "BhenchodForums Sync:  Role '#{g[:discourse_name]}' created!"
            rescue => e
              event.respond 'BhenchodForums Sync:  I dont appear to have rights to create Roles!'
              ...
              event.respond "BhenchodForums Sync:  Role '#{g[:discourse_name]}' already exists!"
            end
          ...
          event.respond "BhenchodForums Sync:  Building user role mapping ..."
          ...
          event.respond "BhenchodForums Sync:  Adding users to roles ..."

          ug_count = ug_list.count
          ug_list.each_with_index do |ug, index|
            event.respond "BhenchodForums Sync:  [#{index + 1}/#{ug_count}] Adding member '#{ug[:discourse_username]}' to '#{ug[:discourse_group_name]}'"
          ...
          event.respond "BhenchodForums Sync:  DONE!"
        else
          event.respond "BhenchodForums Sync:  No users were found in elibigle groups for sync using provided or default criteria!"
        end
      else
        event.respond "BhenchodForums Sync:  No eligible groups for sync using provided or default criteria!"
      end
    end
